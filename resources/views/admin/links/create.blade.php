<x-app-layout>
    <x-slot name="header">
        <h1 class="text-2xl font-semibold text-igp-text">Add Quick Link</h1>
    </x-slot>

    <form method="POST" action="{{ route('admin.links.store') }}" class="max-w-2xl space-y-4 bg-white rounded-lg shadow p-6">
        @csrf
        <div>
        ...
        <div>
            ...
            <input type="url" name="url" value="{{ old('url') }}" required placeholder="https://" class="mt-1 block w-full rounded-md border-gray-300">
        ...
        <div>
            ...
            <input type="number" name="sort_order" value="{{ old('sort_order', 0) }}" min="0" class="mt-1 block w-full rounded-md border-gray-300">
        </div>
